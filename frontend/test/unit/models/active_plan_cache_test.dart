  ...
  group('ActivePlanCache', () {
    ...
    group('Factory constructor confirmedNoPlan', () {
      test('creates cache entry for confirmed no active plan', () {
        ...
        expect(
            cache.lastConfirmedAt.isAfter(beforeCreation) ||
                cache.lastConfirmedAt.isAtSameMomentAs(beforeCreation),
            isTrue);
        expect(
            cache.lastConfirmedAt.isBefore(afterCreation) ||
                cache.lastConfirmedAt.isAtSameMomentAs(afterCreation),
            isTrue);
        expect(
            cache.updatedAt.isAfter(beforeCreation) ||
                cache.updatedAt.isAtSameMomentAs(beforeCreation),
            isTrue);
        expect(
            cache.updatedAt.isBefore(afterCreation) ||
                cache.updatedAt.isAtSameMomentAs(afterCreation),
            isTrue);
      });
    ...
    group('isFresh getter', () {
      ...
      test('handles edge cases around minute boundaries', () {
        ...
        final almostStale =
            DateTime.now().subtract(Duration(minutes: 4, seconds: 59));
        ...
        final definitelyStale =
            DateTime.now().subtract(Duration(minutes: 5, seconds: 1));
    ...
    group('toString method', () {
      test('includes all relevant information', () {
        ...
        expect(
            stringRepresentation, contains('false')); // confirmedNoActivePlan
