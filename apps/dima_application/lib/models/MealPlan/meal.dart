...
class Meal {
  ...
  Meal.create({required this.name, required this.recipe, required this.ingredients, required this.totalMacros});

   factory Meal.fromJson(Map<String, dynamic> json) {
     final List<dynamic> ingredientsList = json['ingredients'] ?? [];
     return Meal.create(
       name: json['meal'] ?? 'Unknown Meal',
       recipe: json['recipe'] ?? '',
       ingredients: ingredientsList.map((ingJson) => Ingredient.fromJson(ingJson)).toList(),
