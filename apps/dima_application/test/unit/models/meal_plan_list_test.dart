  ...
  group('MealPlanList', () {
    ...
    group('Factory constructor fromJson', () {
      ...
      test('handles unknown status values gracefully', () {
        ...
        expect(mealPlanList.items.length, 1);
        expect(mealPlanList.items[0].status, isNull); // Unknown enum values become null
      });
    });
    ...
    group('Factory constructor fromAmplifyModels', () {
      test('creates MealPlanList from Amplify models with active plan detection', () {
        final mealPlans = [
          MealPlan(
        ...
        final mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(mealPlans);
        
        expect(mealPlanList.items.length, 3);
        expect(mealPlanList.activeMealPlan, 'amplify-2'); // ACTIVE plan detected
        expect(mealPlanList.nextToken, isNull);
      });
      ...
      test('uses specified currentMealPlanId over status detection', () {
        ...
        expect(mealPlanList.activeMealPlan, 'specified-2'); // Specified ID takes precedence
      });
      ...
      test('falls back to status detection when specified ID not found', () {
        ...
        expect(mealPlanList.activeMealPlan, 'fallback-1'); // Falls back to ACTIVE status
      });
      ...
      test('handles empty meal plans list', () {
        final mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(<MealPlan>[]);
      ...
      test('creates independent list copy', () {
        ...
        final mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(originalPlans);
    ...
    group('Realistic usage scenarios', () {
      ...
      test('simulates plan activation workflow', () {
        ...
        var mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(plans);
        expect(mealPlanList.activeMealPlan, isNull);
    ...
    group('Edge cases and error handling', () {
      test('handles very large meal plans list', () {
        final largePlansList = List.generate(1000, (index) =>
          MealPlan(
        ...
        final mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(largePlansList);
      ...
      test('handles malformed JSON gracefully', () {
        ...
        expect(() => meal_plan_models.MealPlanList.fromJson(malformedJson), throwsA(isA<TypeError>()));
      });
      ...
      test('handles multiple active plans by selecting first', () {
        ...
        final mealPlanList = meal_plan_models.MealPlanList.fromAmplifyModels(plansWithMultipleActive);
        
        expect(mealPlanList.activeMealPlan, 'multi-active-1'); // First active plan
      });
    });
  });
}