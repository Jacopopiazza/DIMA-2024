...
import 'package:dima_application/models/MealPlanList/meal_plan_list.dart' as meal_plan_models;
...
class MockMealPlanCacheCollection extends Mock implements IsarCollection<MealPlanCache> {}
  ...
  group('MealPlanListCache', () {
    ...
    group('Factory constructor fromDomain', () {
      test('creates cache from MealPlanList with active plan', () {
        ...
        final cache = meal_plan_models.MealPlanListCache.fromDomain(domainList, userId);
      ...
      test('handles multiple meal plans correctly', () {
        final mealPlans = List.generate(5, (index) => MealPlan(
        ...
        final cache = meal_plan_models.MealPlanListCache.fromDomain(domainList, 'user-multi');
        
        expect(cache.userId, 'user-multi');
        expect(cache.currentMealPlanId, 'plan-2');
        expect(cache.allMealPlanIds, ['plan-0', 'plan-1', 'plan-2', 'plan-3', 'plan-4']);
    ...
    group('JSON serialization', () {
      ...
      test('round-trip JSON serialization', () {
        ...
        final recreatedCache = meal_plan_models.MealPlanListCache.fromJson(json);
        
        expect(recreatedCache.userId, originalCache.userId);
        expect(recreatedCache.currentMealPlanId, originalCache.currentMealPlanId);
    ...
    group('Realistic usage scenarios', () {
      test('simulates user meal plan list caching workflow', () {
        ...
        var cache = meal_plan_models.MealPlanListCache.fromDomain(domainList, userId);
      ...
      test('handles plan activation workflow', () {
        ...
        var cache = meal_plan_models.MealPlanListCache.fromDomain(oldDomainList, userId);
        ...
        cache = meal_plan_models.MealPlanListCache.fromDomain(newDomainList, userId);
      ...
      test('manages large meal plan collections', () {
        ...
        final manyPlans = List.generate(planCount, (index) => MealPlan(
          mealPlanId: 'plan-$index',
          userId: userId,
          planName: 'Plan $index',
    ...
    group('Edge cases and error conditions', () {
      ...
      test('preserves order of meal plan IDs', () {
        ...
        final cache = meal_plan_models.MealPlanListCache(allMealPlanIds: orderedIds);
      ...
      test('handles duplicate meal plan IDs', () {
        ...
        final cache = meal_plan_models.MealPlanListCache(allMealPlanIds: idsWithDuplicates);
      ...
      test('handles malformed JSON gracefully', () {
        ...
        expect(() => meal_plan_models.MealPlanListCache.fromJson(malformedJson), throwsA(isA<TypeError>()));
      });
      ...
      test('validates required fields in JSON', () {
        ...
        expect(() => meal_plan_models.MealPlanListCache.fromJson(incompleteJson), throwsA(isA<TypeError>()));
      });
    });
  });
}